...
--*/
{
    ...
    if (!pMmAllocateIndependentPagesEx) {
        for (UINT8 i = 0;
            i < NtosPatternData.BodyFallbackPatternCount; i++) {
            if (HR_ERROR(PeFindSectionMemoryPattern(
                pNtosBase,
                NtosPatternData.FallbackSectionName[i],
                TRUE,
                NtosPatternData.pBodyFallbackPattern[i],
                NtosPatternData.pBodyFallbackMask[i],
                (VOID*)&pMmAllocateIndependentPagesEx))) {
                DBG_BREAK;
                goto aborted;
            }
